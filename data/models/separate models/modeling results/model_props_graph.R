...
barplot(counts, main="",
  		names.arg = c('Adults','Adults', 'Children', 'Children', '', ''), col= cols,
 		legend = c('Lag', 'Spatial',  'Value', 'Random'), density = densities,
 		ylab='Proportion of Participants', cex.axis = 1.2, cex.lab = 1.2,
 		args.legend = c(cex=0.8), cex.names = 0.8)




counts <- c(0, 1, 18/32, 14/32)

counts <- c(0,0,0,0)

counts <- matrix(counts,2)

cols= c('#009966',  '#990066')

densities <- c(100, 30)
barplot(counts, main="",
  		names.arg = c('Adults', 'Children'), col= cols,
 		legend = c('Lag', 'Value'), density = densities,
 		ylab='Proportion of Participants', cex.axis = 1.2, cex.lab = 1.2,
 		args.legend = c(cex=0.8), cex.names = 0.8)
