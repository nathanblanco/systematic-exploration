...
class Input:
    ...
    def draw(self, surface):
        ...
        text = self.font.render(self.prompt+self.value, 1, self.color, (0,0,0))
        ...
        for event in events:
            ...
            if event.type == KEYDOWN:
                if event.key == K_BACKSPACE:
                    # added 10/5 # backspace was not working properly
                    oldvalue = self.value[:-1]
                    self.value = '             '
                    self.draw(surface) 
                    self.value = oldvalue
                ...
                if not self.shifted:
                    ...
                    elif event.key == K_RETURN: self.value += 'R'
                elif self.shifted:
                    if event.key == K_a and 'A' in self.restricted: self.value += 'A'
                    elif event.key == K_b and 'B' in self.restricted: self.value += 'B'
                    elif event.key == K_c and 'C' in self.restricted: self.value += 'C'
